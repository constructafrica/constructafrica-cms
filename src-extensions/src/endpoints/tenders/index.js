...
export default (router, { services, database, getSchema }) => {
  ...
  router.get("/", async (req, res, next) => {
    try {
      ...
      const newsService = new ItemsService("tenders", {
        schema: schema,
      ...
      const transformedNews = news.map((item) => {
        ...
        if (item.featured_image) {
          if (
            typeof item.featured_image === "object" &&
  ...
  router.get("/stats/filters", async (req, res, next) => {
    try {
      ...
      const itemService = new ItemsService("tenders", {
        schema: schema,
        accountability: req.accountability,
      });
      ...
      const idKey = `${
        q === "type" ? "types" : q === "sector" ? "sectors" : "regions"
      }_id`;
