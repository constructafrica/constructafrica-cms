...
export default (router, { services, exceptions, getSchema, database }) => {
  ...
  async function getUserAccessibleFilters(accountability) {
    ...
    if (
      ...
      user.subscription_type !== "projects"
    ) {
      return { regions: [], sectors: [], hasAccess: false };
    }
  ...
  router.get("/", async (req, res, next) => {
    try {
      ...
      const transformedProjects = projects.map((project) => {
        ...
        const originalCountries = project.countries
          ? [...project.countries]
          : [];
      ...
      if (accountability?.user) {
        ...
        finalProjects = await addNotificationsStatus(
          finalProjects,
          accountability.user,
          schema,
          req.accountability,
    ...
    try {
      ...
      if (!q || !validFilters.includes(q)) {
        return res.status(400).json({
          error:
            "Invalid filter parameter. Must be one of: type, sector, sub_sector, region, stage",
        });
      }
