  ...
  <div v-else v-click-outside="closeDropdown" class="m2o-dropdown">
    ...
    >
      ...
      <div class="toggle-icons">
        <v-icon
            v-if="selectedItemData && !disabled"
            name="close"
            small
            clickable
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const relatedPrimaryKeyFieldName = computed(() => {
      ...
      return pkField?.field || 'id'; // fallback to 'id' just in case
    });

    const displayTemplate = computed(() => {
      if (props.template) return props.template;
      if (relatedPrimaryKeyFieldName.value) {
        return `{{ ${relatedPrimaryKeyFieldName.value} }}`;
      }
      return "{{ id }}";
    });
    ...
    const clearSelection = () => {
      ...
      console.log("Emitting value:", valueToEmit, "type:", typeof valueToEmit);
      console.log("Field schema type:", fieldsStore.getField(props.collection, props.field)?.type);
      console.log("Related PK field name:", relatedPrimaryKeyFieldName.value);
      emit("input", null);
    };
    ...
    const selectItem = (item) => {
      ...
      console.log("SELL", selectedItemData.value, itemId);

      // Ensure proper type based on field type
      let valueToEmit = itemId;

      if (itemId !== null && itemId !== undefined) {
        const field = fieldsStore.getField(props.collection, props.field);

        // If the M2O field expects an integer/bigInteger, convert string PK to number
        if (['integer', 'bigInteger'].includes(field?.type) && typeof itemId === 'string') {
          valueToEmit = parseInt(itemId, 10); // or BigInt for bigInteger
        }
            // For UUID fields, ensure we emit a string (no change needed)
        // If somehow a number is passed to a uuid field, convert to string
        else if (field?.type === 'uuid' && typeof itemId === 'number') {
          valueToEmit = itemId.toString();
        }
      }
    ...
    const saveNewItem = async () => {
      ...
      try {
        ...
        if (relatedPrimaryKeyFieldName.value) {
          let itemId = created[relatedPrimaryKeyFieldName.value];

          // Ensure proper type
          const field = fieldsStore.getField(props.collection, props.field);
          // If the M2O field expects an integer/bigInteger, convert string PK to number
          if (['integer', 'bigInteger'].includes(field?.type) && typeof itemId === 'string') {
            itemId = parseInt(itemId, 10); // or BigInt for bigInteger
          }
              // For UUID fields, ensure we emit a string (no change needed)
          // If somehow a number is passed to a uuid field, convert to string
          else if (field?.type === 'uuid' && typeof itemId === 'number') {
    ...
    watch(
        ...
        async (newValue) => {
          ...
          const exists = items.value.find(
              (item) => item[relatedPrimaryKeyFieldName.value] === newValue,
          );
